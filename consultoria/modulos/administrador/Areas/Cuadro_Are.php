<?php 
 ...
 $query="select * from AreaEspecializacion";
...
<head>
  <meta charset="UTF-8">
  <title>Mantenimientos de Áreas</title>
...
<!--
/*function confirmation(codArea) {
  var id=codArea;
  var answer = confirm("¿Deseas Eliminar este Area?")
  if (answer)
  {
    $.ajax({
    url : "librerias/php/Areas/eliminar.php",
    data: {id},
    type: "post",
   ...
   // alert("Bye bye!")
   // window.location = "librerias/php/Areas/eliminar.php";
...
<legend><h3 align=center><p style="font-family: Verdana; color:#0072bb"><strong>MANTENIMIENTO DE ÁREAS DE ESPECIALIZACIÓN</strong></p></h3></legend>

        <div><button class="myButton" id="myButton" style="box-shadow: 0px 1px 0px 0px rgb(151, 196, 254) inset; background: rgb(61, 148, 246) linear-gradient(to bottom, rgb(61, 148, 246) 5%, rgb(30, 98, 208) 100%) repeat scroll 0% 0%; border-radius: 6px; border: 1px solid rgb(51, 127, 237); display: inline-block; cursor: pointer; color: rgb(255, 255, 255); font-family: Arial; font-size: 15px; font-weight: bold; padding: 6px 24px; text-decoration: none; text-shadow: 0px 1px 0px rgb(21, 112, 205);" onclick="showNewAreas()" >Nueva Área</button></div>
        <br>
                <thead>
                    <tr width=90px, height=35px>
            <th style='text-align:center;' width="10%">N°</th>
            <th style='text-align:center;' width="60%">Nombre Área</th><!--Estado-->
