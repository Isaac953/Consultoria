...
//echo "id de la oferta: ".$id;
//echo "<br>id de la asignacion :".$id2;

$consultaOferta="select c.NombreConsultoria, f.NombreEmpresa, f.Nit, f.Correo, f.Monto, p.Nombres, p.Apellidos from Consultoria c, Ofertas f, Asignacion a, Personal p where 
c.Codigoconsultoria=f.Codigoconsultoria and a.CodigoPersonal=p.CodigoPersonal
 and f.CodigoOferta = '$id' and a.CodigoAsignacion='$id2';";
...
<legend><h2 align=center><p style="font-family: Verdana; color:#027BF6"><strong>EVALUACIÓN DE OFERTAS</strong></p></h2></legend>
        ...
        <br>
        <form action="librerias/php/EvaluacionIni/Modificar.php" method="post">
<br>
        <!--TABLA DE CRITERIOS-->
...
<th class="enc" style="background-color: #0072bb; color: white; font-family: Verdana" width="20%">Consultoría</th>
...
<th class="enc" style="background-color: #0072bb; color: white; font-family: Verdana" width="20%">Evaluador</th>
</tr>
<tr>
<?php 
...
echo "<td>".$filaO->Nombres." ".$filaO->Apellidos."</td>";
  }
 ...
 while($fila=sqlsrv_fetch_object($resultado) )
  {
    ...
    $consulta2="select e.CodigoEvalInicialConsultoria, cr.CodigoCriterio, p.CodigoParametrosCriterios, cr.Ponderacion as pondeC, p.Ponderacion as pondeP,
...
and a.CodigoAsignacion='$id2' and e.CodigoOferta='$id' and p.CodigoCriterio='$idC'
;";
    ...
    */
    $resultado2=sqlsrv_query($conexion,$consulta2);
     ...
     {
    echo "<tr><td align='left'><input type='hidden' name='codEvalIni[]' value='$fila2->CodigoEvalInicialConsultoria' size='1' /><input type='hidden' name='parametro[]' value='$fila2->CodigoParametrosCriterios' size='1' />"." ".$fila2->Parametro."</td><td align='center'>".$fila2->pondeP*(100)."%"."</td>";
  echo "<td align='center'><input type='number' class='form-control input-md' required placeholder='nota' style='font-family: Verdana; text-align: center;' name='puntaje[]' value='$fila2->puntajeP' min='1' max='10' size='10' /></td>
  </tr>";
     }
  }
?>
